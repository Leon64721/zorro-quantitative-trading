...
The **sizeof()**  macro ** **that is used in the example above ** **gives  the size of a variable or a struct
instance in bytes. This can be used to initialize structs:
...
 !!  Unlike C/C++, **sizeof(some
struct)** requires that at least one instance of the struct exists. Otherwise **sizeof** will return 0.


 !!  Arrays are internally treated as a pointer to a memory area. So, **sizeof(some array)** and **sizeof(some pointer)** always
evaluates to **4** because that is the size of a pointer.
