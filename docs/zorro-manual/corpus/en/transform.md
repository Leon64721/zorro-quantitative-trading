...
data arrays. See also [Normalization](norm.htm).

## correl(vars Data, int Length, function): var

Spearman correlation of the **Data** series with an arbitrary
**function**. Used for the [CTI](ta.htm),
[CCYI](ta.htm), and [CCYIR](ta.htm) indicators. Source in
...
f.i. for the [markowitz efficient frontier](markowitz.htm) calculation.
...
**Normalize**, or [cdf](cdf.htm) function. The minimum
...
The function internally creates [series](series.htm) (see remarks). Source code in **indicators.c**.
...
The function internally creates [series](series.htm) (see remarks). Source in **indicators.c**.
...
Source
in **ta-lib.zip**. For higher order regression, use [polyfit / polynom](polyfit.htm).
For logistic regression with multiple variables, use the [advise(PERCEPTRON)](advisor.htm) function.
...
**Data** series
was shifted ([+N](series.htm)), add the offset **N** to
...
Source in **ta-lib.zip**. See also [Percentile](#percentile).
...
**1..TimePeriod-1**. See the **[RandomWalk](scripts.htm)**
...
[
Financial Hacker](http://www.financial-hacker.com/is-scalping-irrational/) blog; source in **indicators.c**.
...
### Standard parameters:
		...
		**0** for using a default period.
		 ...
		 | **Data**
		 | A data [series](series.htm), in reverse time order,
		usually  derived from
		the price functions **[price(), priceC()](price.htm)**
...
[Spectral filters](filter.htm), [indicators](ta.htm),
