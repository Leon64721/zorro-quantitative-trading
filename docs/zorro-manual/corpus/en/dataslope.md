...
Splits the [WFO](numwfocycles.htm) simulation in a training period (given in percent) and a following test period. F.i. when set at **60**, the training period has a length of **60%** and the test period has a length of **40%**. This
works with WFO ([NumWFOCycles](numwfocycles.htm) > 1) or without WFO
([NumWFOCycles](numwfocycles.htm) == 1) and ensures that the test uses out-of-sample data.
