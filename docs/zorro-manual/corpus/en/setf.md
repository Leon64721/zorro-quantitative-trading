...
Identical to **Mode |= (Flag)**. Sets a **Flag** or flag combination in the variable
**Mode** without affecting the other flags. For
instance, **setf(TradeMode,TR_PHANTOM)** sets the **TR_PHANTOM**
flag for the [TradeMode](trademode.htm) variable.

## resf (int Mode, int Flag)

Identical to **Mode &= ~(Flag)**. Resets a **Flag** or flag combination in the variable
**Mode**.

## isf (int Mode, int Flag): int

Identical to **(Mode&(Flag))**. Evaluates to nonzero when the a **Flag** in the **Mode**
...
[set](mode.htm), [PlotMode](plotmode.htm),
[TrainMode](opt.htm), [TradeMode](trademode.htm)
