...
Fuzzy comparison, equivalent to the binary **[==](comparisions.htm)**, **
[>](comparisions.htm)** and [**<**](comparisions.htm)**** operators.
...
### Parameters:
      ...
      | **a, b **
      | Fuzzy truth values, **0.0 .. 1.0**


      | **Data, Data1, Data2 **
      | Data [series](series.htm).
...
Defuzzyfication, converts a fuzzy truth value to binary **true** or **false**.
  Use this function to convert the result of a fuzzy logic expression back to binary logic.
...
## FuzzyRange
 ...
 Determines the 'fuzziness' range of the input data (default: **0** = no fuzziness). When comparing two variables, the truth value goes from **0** to **1** within that range. Set this to a small fraction of the price volatility, or f.i. to **0.1*PIP** for comparing moving averages, or to **0.1** for comparing percentage values.
The smaller this value, the 'less fuzzy' is the logic. At the default value **0** the logic is binary.
The **FuzzyRange** variable is also used for classifying [signal patterns](advisor.htm) for price action trading.
