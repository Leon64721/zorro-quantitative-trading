...
Assign a user-supplied function to a particular event, thus replacing its
default function. Functions with the same
name as the event are automatically assigned by default at script start.
### Parameters:


		 | **Event**
		 | **1** [objective](objective.htm), **2**
		[evaluate](evaluate.htm), **3**
		[tick](tick.htm), **4** [tock](tick.htm),
		**5** [manage](trade.htm), **6**
		[click](panel.htm), **7** [
		neural](advisor.htm), **8** [bar](bar.htm), **9**
		[callback](tick.htm), **10** [parameters](objective.htm),
		**11** [error](errors.htm), **12**
...
- **call(1, ...)** can be used to prevent that the called
	...
	a trade, or
	writing to a file in a [click](panel.htm) or [
	callback](tick.htm) function.
...
### See also:
  ...
   [run](run.htm), [lock](lock.htm),
[quit](quit.htm), [version](version.htm),
[wait](sleep.htm), [click](panel.htm)
