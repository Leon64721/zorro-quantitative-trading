...
[Oanda](oanda.htm), apply virtual hedging automatically to all trades.
...
Virtual hedging affects [performance parameters](performance.htm).
...
### Range:
		...
		the [run](run.htem) function are collected and result in a
		...
		compliant). Like Hedge mode **5**, but pool trades are not
		automatically snychronized at any bar, but only when the [tradeUpdate](tradeupdate.htm) function
		is called.
...
- Virtual hedging requires [Zorro S](restrictions.htm). The [performance report](performance.htm) and the
	[Total](winloss.htm) statistics reflect the pool trades, the
	[Long/Short](winloss.htm) statistics reflect the virtual and phantom trades.
	...
	phantom trades for the purpose of [equity curve trading](trademode.htm) also contribute to the Long/Short statistics,
...
- **Hedge = 5** is preferable to **Hedge = 4** since
	...
	**[TradeTest](scripts.htm)** script. Start a session,
...
- The number of open net lots of pool and phantom trades can be evaluated
	with the [LotsPool](winloss.htm) and [LotsPhantom](winloss.htm)
...
- Opening pool trades can be prevented by setting [Lots](lots.htm)
...
- Pool trades have no profit target and no [TMF](trade.htm), but
...
- Pool trades appear in [trade enumeration loops](fortrades.htm) and
	can be identified by **[TradeIsPool](trade.htm)**.
...
- Pool trades that were rejected by the broker will be re-entered at every
	bar until [LotsPool](winloss.htm) and [LotsPhantom](winloss.htm)
...
- In virtual hedging mode, the [result window](trading.htm) displays
	only open pool trades and pending phantom trades. When [algo
	identifiers](algo.htm) are used, pool trades are displayed either with the identifier
...
- Hedging is prohibited for US based accounts due to NFA Compliance Rule 2-43(b).
	Such accounts normally require the **[NFA](mode.htm#nfa)**
...
[NFA](mode.htm), [enterLong/Short](buylong.htm),
[LotsPool](winloss.htm), [Phantom trades](trademode.htm),
[tradeUpdate](tradeupdate.htm)
