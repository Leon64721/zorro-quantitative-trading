...
Instead of fixed bar periods, bars can also cover fixed price ranges or can be defined by
any other criteria.
Some traders believe that those special bars help their trading, because they
make the chart look more predictive. Zorro supports a user-defined **bar** function
...
 
## bar(var *Open, var *High, var *Low, var *Close, *var *Price, DATE Start, DATE Time*): int

User-supplied function for defining a [bar](bars.htm). Used to create special bar types or to replace the mean price by a different algorithm
such as [VWAP](ta.htm). The
**bar** function is automatically called whenever a new price quote arrives or a price tick is read from a historical file. It can modify the candle and determine
if and when the bar ends.
...
****1 **- close the bar now, and call the **bar** function
again at the next price tick.

**4** - keep the bar open, and call the **bar** function
again at the next price tick.


**8** - call the **bar** function only at the normal
end of the [BarPeriod](barperiod.htm)..
...
- Modified prices  also determine the corresponding price functions ([priceO](price.htm), [priceH](price.htm), [priceL](price.htm), [priceC](price.htm), [price](price.htm))
...
- Most special bars, such as Haiken Ashi or Range/Renko bars, are
			**affected by any preceding bar** and thus
			...
			first bar. Their candles therefore depends on start date, lookback period, bar period, bar
			offset, time zone, or any other settings that affect the first bar.

- [BarPeriod](barperiod.htm) has
			...
			set [BarPeriod](barperiod.htm) to the approximate average bar
			duration that is displayed in the
			[performance report](performance.htm). If you don't know
			it, set
...
- In **multi-asset strategies**, or in strategies with multiple bar
			...
			[series](series.htm) must be static and shifted by script.
			An example for a multiple asset strategy with Renko bars can be
			found in the **[SpecialBars](scripts.htm)** script.
