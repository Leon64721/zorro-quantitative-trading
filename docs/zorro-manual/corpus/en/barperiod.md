...
theoretical minimum bar period is one second (**1./60**) for normal scripts, and 1 ms (**0.001/60**) for [HFT simulation](fill.htm).
Typical price curve inefficiencies disappear on low time frames, so don't go
below **60** for indicators, filters, or analysis functions. Use **1440** for daily and **10080** for weekly bar periods. The bar period is the basic  time frame in the script. It determines the width of a candle in the chart, and the frequency of the [run](run.htm) function that is called after every bar. If not explicitly set up in the script, it can be determined with the [Period] slider.
User-defined bars (see [bar](bar.htm) function) have no fixed bar period; in that case **BarPeriod** should be set to the average duration of a bar.
...
use **BarOffset** to shift the bar begin to the desired trading time
of the day; the **940** default sets it to 15:40 in the bar
[time zone](assetzone.htm). **BarOffset** is also automatically set by **[NumSampleCycles](numsamplecycles.htm)**
...
Time frame in bars (default = **1**) used for all subsequent [price](price.htm), [time](month.htm), [series](series.htm), [advise](advisor.htm), and [trade](buylong.htm) calls. This variable can be set within a strategy for using multiple time frames,
...
shifting of [series](series.htm); price and other functions are
unaffected. See remarks and example; see also [frameSync](frame.htm)
and [AssetFrame](assetzone.htm).

## FrameOffset

Time frame offset in bars (default = **0** = no offset) used for all subsequent [price](price.htm), [time](month.htm), [series](series.htm), and [trade](buylong.htm)  calls; must be smaller than **TimeFrame**. When fixed time frames are used, this variable determines  the bar number within a time frame when series are shifted and trades are executed.
...
- In [Test] mode, bar generation is based on the
  ...
  the middle price instead, you can use the [TickFix](ticktime.htm)
...
- On short bar periods it is recommended to set the [TICKS](mode.htm) flag and use high resolution [.t1 price data](testing.htm#m1t1). It's mandatory on bar periods of less than a minute
  - not only for the backtest, but also for filling the lookback period in live
  trading. For this, set [History](script.htm) to **".t1"** and **BarPeriod** to the number of seconds divided by 60, f.i. **BarPeriod = 5./60.** for 5 seconds. Mind the decimal; **5/60** would be considered an **int** and thus evaluate to **0**. For extremely short bar periods, make sure that [TickTime](ticktime.htm) is always less than a bar period.
...
- **BarPeriod** and **BarOffset** affect the
	...
	[asset](asset.htm), must not change afterwards, and must not be
...
- All time zone settings assume UTC time for price history. If price
	...
	either convert the timestamps to UTC or use [BarZone](assetzone.htm).
...
- **Barperiod** determines the bar chart and the frequency of the
	  [run](run.htm) function, while **TimeFrame** affects the
	  [price](price.htm) functions, the shifting of
	  data
	  [series](series.htm), and the bars at which
	  [advise](advisor.htm) and [trade](buylong.htm) calls are executed.

- Commands to enter or exit trades in the [run](run.htm)
	  ...
	  **TimeFrame** has ended at the current bar. But trade entry,
	  ...
	  [run](run.htm) function itself is executed at any bar period,
	  but the [frame](frame.htm) function can be used for restricting
...
-
	  **TimeFrame** is normally not
	  ...
	  [frameSync()](frame.htm) function. For aligning daily time
	  frames to a certain market hour in a certain time zone, use the [AssetFrame](assetzone.htm)
	  variable.

- **TimeFrame** has no effect on variables in bar period units, f.i.
	  [LookBack](lookback.htm), [UnstablePeriod](lookback.htm), [LifeTime](timewait.htm), or
	  [WaitTime](timewait.htm). For extending those time periods, multiply the variable value with the number of bars per time frame.
...
- Recursive indicators from the TA-Lib such as **EMA** or **ATR** require
	  [UnstablePeriod](lookback.htm) to be adapted to their maximum
	  time frame. 
...
[Bars and Candles](bars.htm), [NumBars](numbars.htm),
[SampleOffset](numsamplecycles.htm), [LookBack](lookback.htm), [dayHigh](day.htm), [Date](date.htm), [BarMode](barmode.htm), [AssetFrame](assetzone.htm), [frameSync](frame.htm), [bar](bar.htm)
