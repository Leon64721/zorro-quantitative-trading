...
platforms are usually designed for manual trading, Zorro is specialized on
research and algorithmic trading in automated or semi-automated modes. It can do
a lot more than mere trading platforms. Technically
it's a front end to a C and C++ compiler with a set of modules for time series analysis,
...
algorithms, optimize and backtest systems with multiple methods, and deploy them
...
The answer on questions like '***Does Zorro have X?***' (where
***X*** is a market scanner, a stock exchange crash
predictor, a ChatGPT-controlled trading system, or whatever trading or analysis feature), the answer is most likely: '***Yes.
Just write a script for it***'. Everything you're doing with Zorro is based on a **script**,
and all trading, analysis, and AI features are functions called in your script. There are many
...
[books](links.htm), [
online courses](https://zorro-project.com/docs.php), and also a [short tutorial](tutorial_var.htm) in
this manual.
...
We admit: the user interface is more functional than pretty. But is has
anything that you'll ever need for research and trading. For settings specific
to your script, you can add
[your
own user interface](panel.htm) with as many colorful fields and buttons
as you want. The default control panel is designed for minimum space (~ 300x600 pixels),
...
### Strategy
				...
				**folder. See [scripts](scripts.htm) for
				included scripts and the [tutorial](tutorial_var.htm) for writing
				your own scripts.
...
Select the primary asset. You can add any asset - crypto,
				...
				through [asset lists](account.htm). 
...
[Bar period](bars.htm) in minutes;
...
### Buttons
				 ...
				 | Open the selected script in the [editor](npp.htm).
				Reset all settings by the previous run.
				 ...
				 | [Test](testing.htm) the script with historical
				price data in single-step, tick-based, or walk-forward mode.


				 | Train
				 | [Train](training.htm) the script by
				[optimizing](optimize.htm) parameters, generating
				trade [rules](advisor.htm), or training neural networks.
...
Run the script live with broker connection.
				[Trade](trading.htm) assets and start earning (or losing) money.
				 ...
				 | Open the [chart
				viewer](chart.htm) and the editor with the log and
				[performance analysis](performance.htm).
...
- For live trading, any decent Windows PC with
	...
	in the Cloud](vps.htm). If you run many Zorro instances on several VPS with
	different brokers and accounts, you can
	observe the statuses of all of them on your PC at home with the
	[ZStatus](trading.htm#zstatus) script.
  You also need an
	account with an [online broker](brokers.htm) (or several accounts
...
 
	...
	C++, get the latest [Visual Studio Community Edition](dlls.htm)
	from Microsoft and install it with its C++ environment. For using deep
	learning libraries, install [R](rbridge.htm) or
	[Python](python.htm).
...
- **Let Zorro do something:** Start Zorro
	...
	of included scripts](scripts.htm).
 

- **Learn trading strategy development:**  Work through the
	[quick tutorial](tutorial_var.htm), or get the [Black
	Book](links.htm) for a thorough intoduction to strategy programming in C. If you have a question or an issue with Zorro, ask on the
	[
	user forum](http://www.opserver.de/ubb7/ubbthreads.php?ubb=cfrm&c=1). For individual help, subscribe a support ticket on the
	[Zorro support
	page](http://zorro-project.com/docs.php). If you need help with developing a trading strategy, contact our
	[system development
...
- **Earn virtual money:** Open a demo
	...
	select a **[Z](zsystems.htm)**[
	strategy](zsystems.htm), and click
...
- **Earn real money: **The free Zorro version is fully functional
	...
	one of the included strategies, please read the [Z systems](zsystems.htm#income)
	chapter from top to bottom. Open a real money account with the broker of your
	choice (for finding a broker, read the [Brokers](brokers.htm) page).
	Some brokers offer a [free Zorro S subscription](restrictions.htm)
...
[Content](content.htm), [What's New](new.htm),
[Algo Trading Principles](strategy.htm), [Tutorial](tutorial_var.htm),
[Z Strategies](zsystems.htm)
