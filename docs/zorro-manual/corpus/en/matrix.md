...
The matrix or vector type **mat** is a pointer to the.**MATRIX**
struct.
...
Sorts all rows of the matrix by the given column in ascending or descending
order, and returns the sorted matrix.
...
Saves the matrix **X** to a CSV file for further evaluation with
...
- Matrices are created in the **INITRUN** and released after the **EXITRUN**. Just like  [series](series.htm), **matrix()** creation must happen in
  the same order at any bar, preferably at the begin of the **run** function.
