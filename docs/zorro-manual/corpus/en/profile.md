...
Generates a price difference profile starting at the current bar.
This function is normally called by a trade signal for checking its predictive
power. It produces a histogram of
the mean price movement following the signal. The red bars in the histogram are the averaged
mean price difference to the
entry point in pips, the blue/grey bars are their standard deviations, divided by 4 for better scaling.
...
Generates a Maximum Adverse Excursion / Maximum Favorable Excursion graph after the test run. The Maximum Adverse Excursion
(MAE) is the worst drawdown of an individual trade, the difference of its highest interim profit to its worst interim loss.
The Maximum Favorable Excursion (MFE) is the opposite. The graph displays any trade as a dot in a chart showing its MAE
or MFE on the x axis and its profit on the y axis in pip units or in percent of
the price. Winning trades are green, losing trades are red. This graph can be used for analyzing the exit behavior of the trades and finding the optimal stop or take profit distances.
...
### Parameters:
     ...
     | **value**
     | Value to be evaluated, for instance [price(0)](price.htm),
	**Spread**, [Equity](balance.htm), etc..
...
On random data, such autocorrelations should be near zero for all time lags. But if the data is non-random, one or more of the autocorrelations will be significantly non-zero.
...
### Parameters:
     ...
     | **Value1**
     | First variable to be evaluated, for instance [price(0)-price(1)](price.htm).
...
Plots a heatmap in the **EXITRUN** from the given **Matrix**
or the **Data** array of size  **Rows*Cols**, usually a correlation matrix. The matrix elements must be in the** 0..1** range, and are displayed as  squares with a color range from blue (**0**) to red (**1**).
Since the heatmap is simulated with colored dots, you might need to scale the
chart before exporting so
that it looks nice. Examples of heatmap usage can be found on
[
Financial Hacker](https://financial-hacker.com/get-rich-slowly/). A different type of heatmap - a contour chart - can be
plotted with the [dataChart](data.htm) function.
...
Compares the equity curve with a benchmark. Call this function at any
[run](run.htm) for plotting the buy-and-hold profit curve of the given **
Benchmark** asset - for instance, **"SPY"** - in the given
**Color**. The position size is detemined by [Leverage](pip.htm)
and [
Capital](lots.htm). If **Capital** is zero, the position size is given by
...
Like [plot](plot.htm), but plots a two-color curve. **Select**
can be a boolean expression; at **true** (nonzero) the curve is
plotted in **Color1**, otherwise in **Color2**.
 
...
- Price profile or distribution histograms should
  normally only be plotted for a single asset and algorithm, since every market and every system will generate a different profile. In a portfolio strategy, temporarily disable the other assets and algos for plotting a profile of a component.

- Only one histogram can be plotted, not several at the same time;  unused plot commands must be commented out for not interfering with the active chart.
  Histograms are only plotted in [Test] mode, not in [Train] and [Trade] mode. 

- The size and resolution of the histogram can be set up with the [plot parameters](plotbars.htm).
	Label spacing at the x axis can be set up with **PlotLabels**.
...
- When developing a strategy, it is  helpful to examine the price difference profile at every trade
  entry bar for verifying the trade entry rules and for determining the exit rules.
