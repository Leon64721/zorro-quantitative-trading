...
The [is()](mode.htm) function is used for reading the state of a
...
The script is doing a [Test],  [Train], or [Trade]
run (see also [Button](button.htm)).
...
The script is running on a [Zorro S](restrictions.htm) version.
See also [version()](version.htm).
...
First initial run of the script, in the case of multiple simulation runs due to training cycles or [NumTotalCycles](numtotalcycles.htm). While  **INITRUN** and **FIRSTRUN** is set at the start of any simulation run, **FIRSTINITRUN** is only set at the
first initial run.
It is not set in any subsequent training or test cycles.

## EXITRUN

Last run of the simulation. All trades are closed. Can be used to calculate results, aside from the [evaluate](evaluate.htm) or [objective](optimize.htm) functions.
If a live session or simulation is aborted with [Stop] or
[quit(0)](quit.htm), no **EXITRUN** is performed. 
...
determine if an asychronous process, f.i. triggered by a [click](panel.htm)
or [callback](tick.htm)
function, is allowed to access the broker API or global variables. See also
[call()](call.htm). 

## SLIDERS

One of the [sliders](slider.htm) has been moved. Reset this flag with
...
The script is currently generating [parameters](optimize.htm) in [Train]
mode.
...
The script is currently generating [rules](advisor.htm) or machine
...
The script runs in single step mode. For details see
[debugging](chart.htm).
...
The script called the [asset](asset.htm) or [
assetAdd](asset.htm) function.
...
[System flags](mode.htm), [isf](setf.htm),
[Button](button.htm)
