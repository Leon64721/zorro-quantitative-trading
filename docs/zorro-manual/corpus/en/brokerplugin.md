...
Zorro supports most major [brokers](brokers.htm) either with a direct
connection, or by connecting through a broker-supported platform, for instance with
the [MT4/5 bridge](mt4plugin.htm) or the [Sierra
bridge](sierra.htm). A direct connection to a broker API, exchange, market data feed, or
...
can be implemented with an optional [broker command](brokercommand.htm).
...
a small script is sufficient (see [assetHistory](loadhistory.htm)).
...
proper C++ written plugins with a free [Zorro S](restrictions.htm) subscription,
...
code of some other plugins developed by Zorro users is available on the GitHub pages
of their authors.


An article about writing a broker plugin can be found on
[
Financial Hacker](https://financial-hacker.com/crypto-trading-with-rest-part-1/).


The source code of some included broker plugins is available on request to Zorro S
...
[VC++ project setup](dlls.htm#project). Zorro functions, such as for
...
On
[
Financial Hacker](https://financial-hacker.com/crypto-trading-with-rest-part-1) you can find a step by step instruction for implementing a
REST API DLL.
...
- **Simulation** is a simple 'broker simulation' plugin that
	...
	Use it in [NFA](mode.htm#nfa) mode.
...
[Lots](lots.htm) and [Stop](stop.htm) value set up with the
sliders.
...
[Update Stop] - Sets the [Stop](stop.htm)
of all open positions to the value (in Pips) set up with the slider. Stop adjustment
is not supported by some brokers. Due to [StopFactor](stop.htm), the
...
### Parameters:
		...
		**Progress** is a pointer and a **[
		callback](tick.htm)** function exists in the script, it is called and the
...
### Parameters:
		...
		filled with the account id from the [account list](account.htm).
...
**2** when the connection is ok and the
...
- If the server time is returned, but does not change for several minutes,
	Zorro assumes that the broker server is offline. It then displays **"Offline"**
	in the server window, and suspends trading and requesting price quotes.
...
retrieved in [TickTime](ticktime.htm) intervals or when **BrokerProgress**
was preciously called by the plugin. Other asset data is retrieved once per bar.

### Parameters:


		 | **Asset**
		 | Input, asset symbol for live prices (see [Symbols](symbol.htm)).
		...
		struct in **BrokerHistory2** (see below)..
		 ...
		 | **pPip**
		 | Optional output, size of 1 [PIP](pip.htm), f.i. 0.0001 for
		EUR/USD.


		 | **pPipCost**
		 | Optional output, cost of 1 PIP profit or loss per
		[lot](lots.htm#lot), in units of the account currency. If not
		...
		[lot](lots.htm#lot) of the asset. For currencies it's usually
		...
		[lot](lots.htm#lot) of the asset in units of the account currency.
...
**0** otherwise. An asset that returns **0** after subscription
will trigger [Error 053](errors.htm), and its trading will be disabled.
...
- If parameters are not supplied by the broker
	...
	[asset list](account.htm). Only price and spread must always be returned
	when the **pPrice** and **pSpread** parameters are
	nonzero.

- For receiving streaming price data, get the
	Zorro window handle from the [SET_HWND](brokercommand.htm) command
...
[assetHistory](loadhistory.htm) function and at the begin of a trading
session for filling the [lookback](lookback.htm) period.
...
### Parameters:
		...
		Symbols](symbol.htm)).
		 ...
		 | **tStart**
		 | Input, UTC start date/time of the price history (see **BrokerTime**
		about the **DATE** format). This has only the meaning of a
		seek-no-further date; the relevant date for the begin of the history is
		**tEnd**.
		...
		must convert it to UTC.
		 ...
		 | **nTickMinutes**
		 | Input, time period of a tick in minutes. Usual values are **0**
		for single ticks (for T1 or T2 data; all prices of a **T6**
		struct get the tick price), **1** for one-minute
		(M1) historical candles, or a larger value for low-resolution data.


		 | **nTicks**
		 | Input, maximum number of ticks to be filled; must not exceed the number
		returned by **brokerCommand(GET_MAXTICKS,0)**, or **300**
		otherwise.


		 | **ticks**
		 | Output, array of **T6** structs (defined in **include\trading.h**)
		to be filled with the ask prices, close time, and additional data if available,
		such as historical spread and volume. See [history](history.htm)
		for details. The **ticks** array is filled in reverse order
		from **tEnd** on until either the tick time reaches **
		tStart** or the number of ticks reaches **nTicks**,
		whichever happens first. The most recent tick, closest to **tEnd**,
		is at the start of the array. In the case of **T1** or
		**T2** data, or
...
limit. Also used for [NFA compliant](mode.htm#nfa) accounts to close
a position by opening a new position in the opposite direction. The order type (FOK,
IOC, GTC) can be set with [SET_ORDERTYPE](brokercommand.htm) before.
Orders other than GTC are cancelled when they are not completely filled within the
[wait time](brokercommand.htm) (usually 30 seconds).

### Parameters:


		 | **Asset**
		 | Input, asset symbol for trading (see [Symbols](symbol.htm)).
		...
		of [Lots](lots.htm) multiplied with the [LotAmount](pip.htm).
		...
		when [StopFactor](stop.htm) was set, or **0** for
		...
		[OrderLimit](stop.htm), or **0** for market orders.
		Can be ignored if limit orders are not supported by the API. 


		 | **pPrice**
		 | Optional output, the average fill price if the position was partially or fully
		filled. If no price was set, Zorro assumes the current price.
...
- **0** or **1** when the order was rejected or
	...
	time](brokercommand.htm) (adjustable with the [SET_WAIT](brokercommand.htm) command).
...
- **-1** when the trade or order identifier is a UUID that is
	then retrieved with the [GET_UUID](brokercommand.htm) command.

- **-2** when the broker API did not respond within the
	[wait time](brokercommand.htm), so the order state is unknown. The plugin must then cancel
	the order. Zorro will display a "possible orphan" warning.
...
than 1 pip, or when [contractUpdate](contract.htm) or
[contractPrice](contract.htm) is called for an option or future trade.

### Parameters:


		 | **nTradeID**
		 | Input, order/trade ID as returned by **BrokerBuy**, or **-1** when the trade UUID was set before with a
		[SET_UUID](brokercommand.htm) command.


		 | **pOpen**
		 | Optional output, the average fill price if the
		trade was partially or fully filled. If not available by the API, Zorro
		will estimate the values based on last price and asset parameters.
		...
		If not available, Zorro will estimate the swap from the asset
		parameters.
...
limit price. If partial closing is not supported, **nAmount** is ignored
and the trade is completely closed. Only used for not [NFA compliant](mode.htm#nfa)
accounts that support individual closing of trades. If this function is not provided
or if the [NFA](mode.htm) flag is set, Zorro closes the trade by calling
**BrokerBuy2** with the negative amount and with **StopDist**
at **-1**.
...
### Parameters:
		...
		**-1** for a UUID to be set before with a
		...
		[OrderLimit](stop.htm), or **0** for closing at
...
brokerCommand](brokercommand.htm)** function for setting special modes or retrievong
...
- If optional functions are not implemented
	...
	**BrokerTrade** is not implemented and thus the fill amount and
	fill price of [GTC](TradeMode.htm) orders cannot be traced, Zorro
...
- If **BrokerTrade** is implemented, it is automatically called
	...
	[contractUpdate](contract.htm) or [contractPrice](contract.htm),
	and is preceded by a [SET_SYMBOL](brokercommand.htm) command with
...
- For a script with low-level broker access, you can call the above functions
	...
	with **LoadLibrary** or **[DefineApi](litec_api.htm)
	**as described under [DLL / API implementation](litec_api.htm).
...
- For asynchronously triggering a price quote request, send a **WM_APP+1**
	message to the window handle received by the [SET_HWND](brokercommand.htm)
	command. For triggering the [callback](tick.htm) function, send a
...
[Brokers](brokers.htm), [Symbols](symbol.htm),
[brokerCommand](brokercommand.htm), [enter](buylong.htm),
[order](order.htm), [DLL](litec_api.htm), [
IB](ib.htm), [FXCM](fxcm.htm), [Oanda](oanda.htm),
[MT4](mt4plugin.htm)
