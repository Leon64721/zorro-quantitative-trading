...
internal [series](series.htm) (see remarks). Source code in **indicators.c**. 
...
it was never true. This function internally creates [series](series.htm)
...
occurrence. This function internally creates [series](series.htm)
(see remarks). Source code in **indicators.c**.
...
**c** - any integer or boolean expression.
**Data**
- data series of [LookBack](lookback.htm) elements.
**n**
...
- When functions are used for **x** and **y**, be
	...
	regardless of the state of **c**. Use [if](if.htm) for
	executing functions depending on a condition.

- Some  functions internally create [data series](series.htm), and thus
	must be called the same number of times and in the same order at any bar.
...
[abs](abs.htm), [min](min.htm), [max](min.htm),
[between](between.htm), [sign](sign.htm),
[clamp](clamp.htm), [if](if.htm), [
fix0](invalid.htm)
