...
handling combinations of [options](contract.htm), such as Spreads, Straddles,
...
[
Financial Hacker](http://www.financial-hacker.com/algorithmic-options-trading/).
...
passed to [enterLong](buylong.htm). 
...
[contractPrice](contract.htm) should have been called before; the other contract
...
[contractPrice](contract.htm) must have been called before; the other contract
parameters don't matter. Multiplier and transaction costs are not included. Source code in **contract.c**.
...
contracts of the combo must be set, so [contractPrice](contract.htm)
...
and **
20** for a 4-contract combo such as a condor (2 calls and 2 puts). Source code in **contract.c**.
## comboMargin (int
...
[
US Options Margin Requirements](https://www.interactivebrokers.com/en/index.php?f=26660&hm=us&ex=us&rgt=1&rsk=0&pm=1&rst=101004100808) page. Supports single calls and puts, call
...
- Combo trades should be opened or closed in the order of legs, starting
	...
	internally uses the [SET_COMBO_LEG](brokercommand.htm) command,
...
- [Order limits](stop.htm) are normally interpreted by the
	broker as a price limit of the whole combo. The [Stop](stop.htm)
...
- Combos orders can take several minutes for being executed, even at
	market. For this reason it is recommended to use [GTC](trademode.htm)
	orders and/or to increase the wait time with the [
	SET_WAIT](brokercommand.htm) command.

- If a combo order is not opened by the broker, the [
	enter](buylong.htm) call with the last leg will fail. In that case cancel the
	preceding legs with the [cancelTrade](selllong.htm) function.
...
- The current combo remains valid until the next **combo()**
	or [contractUpdate](contract.htm)**()** call.

- Use the [algo](algo.htm) function for getting separate
	combo-specific statistics. For determining the current profit/loss of a
	combo position, use a [trade loop](fortrades.htm) and sum up all
	[TradeProfit](trade.htm) of the same [
	TradeAlgo](trade.htm). Trades of the same combo are listed in sequence in the trade
	loop.
...
[contract](contract.htm), [contract variables](contracts.htm),
[Workshop 8](tutorial_options.htm)
