...
## pow(var x, var y): var
  ...
  An error occurs if x is zero and y
  is less than or equal to 0, or if x is negative and
y has a fractional part. Note that
  x*x is the same, but is computed faster than
**pow(x,2)**         .
### Parameters:
...
[](avar-exp.htm)[exp](avar-exp.htm),
[log](avar-log.htm), [sqrt](avar-sqrt.htm)
[► latest
